...
class VisitorController extends BackendController
{
    ...
    public function store(VisitorRequest $request)
    {
        ...
        // Optional image optimize for public/uploads media (ignore failures)
        try {
            $media = $visitingDetail ? $visitingDetail->getFirstMedia('visitor') : null;
            if ($media) {
                $path = $media->getPath();
...
public function update(Request $request, VisitingDetails $visitor)
{
    $visitingDetail = $this->visitorService->update($request, $visitor->id);

    // Optional: Image optimization if image exists
    try {
        $media = $visitingDetail ? $visitingDetail->getFirstMedia('visitor') : null;
        if ($media) {
            $path = $media->getPath();
        ...
        // silently ignore optimizer error
    }
