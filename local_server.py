...
def check_prerequisites() -> None:
    ...
    if not (APP_DIR / "vendor" / "autoload.php").exists():
        die("Composer vendor/ folder missing. Project vendor must be present.")
    if not HEADLESS:
...
def sqlite_already_seeded() -> bool:
    """True if local SQLite already has core seed data (roles)."""
    ...
    try:
        ...
        _log(f"sqlite_already_seeded check failed: {exc}")
        return False
...
def first_time_setup() -> None:
    ...
    # Migrate schema, then seed (seeders are idempotent / safe to re-run)
    result = run_php(["artisan", "migrate", "--force"], check=False, capture=True)
    ...
    if result.returncode != 0:
        if not HEADLESS and out:
            print(out)
        die("Database migrate failed. See output above / runtime/server.log.")

    seed = run_php(["artisan", "db:seed", "--force"], check=False, capture=True)
    seed_out = ((seed.stdout or "") + "\n" + (seed.stderr or "")).strip()
    if seed_out:
        _log(seed_out[-8000:])
        if not HEADLESS:
            print(seed_out)
    if seed.returncode != 0:
        # Fallback: if unique data already exists, continue when roles are present
        if sqlite_already_seeded():
            if not HEADLESS:
                info("Seed reported errors but roles already exist — continuing")
            _log("db:seed failed but sqlite_already_seeded=True; continuing")
        else:
            die("Database seed failed. See output above / runtime/server.log.")
    elif not HEADLESS:
